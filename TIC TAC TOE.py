...
def set_tile(row,column):
    ...
    if board[row][column]["text"] != "": #it use for not change spot if it alreay exist
        return

    board[row][column]["text"] = curr_player #mark the board

    if curr_player == playerO: #players switching
        curr_player = playerX
    else:
        curr_player = playerO
...
def check_winner():
    ...
    for row in range(3): #check horizontal 3 row
...
frame = tk.Frame(window) #frame inside the window
label = tk.Label(frame, text=curr_player+"'s turn",font=("Consolas",20) ,background=color_grey, foreground="white")
label.grid(row=0,column=0, columnspan=3,sticky="we")

for row in range(3): #this for loops use to convert the tiles to buttons
    for column in range(3):
        board[row][column] = tk.Button(frame,text="",font=("Consolas",50,"bold"),background=color_grey,foreground=color_blue,width=4,height=1,command=lambda row = row, column = column:set_tile(row,column))
        board[row][column].grid(row = row+1,column = column)
button = tk.Button(frame,text = "restart",font=("Consolas",20),background=color_grey,foreground="white",command=new_game)
...
#formula for open tic tac toe in center of the screen formula="(w)x(h)+(x)+(y)"
